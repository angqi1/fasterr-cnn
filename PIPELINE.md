...
| FP16_320h（低分辨率） | 320×960 | FP16 | **62ms** | **1.63x（快39%）** |
| FP16_375h+cuBLASLt | 375×1242 | FP16 | **90ms** | 1.12x（快11%）|

#### ⑥ 检测精度对比（KITTI 100张，IoU≥0.3，thr=0.30）

| 引擎 | Recall | Precision | F1 | car | truck | pedestrian | bus/bicycle |
|------|--------|-----------|-----|-----|-------|-----------|-------------|
| FP16_375h（基准） | 67.4% | 47.0% | 55.4% | 78.1% | 66.0% | 43.1% | 0% |
| **FP16_320×960** | **70.3%** | **49.9%** | **58.4%** | **82.1%** | **69.8%** | 41.2% | 0% |

**结论：320×960 精度不降反升（F1 +3%）**，可安全用于生产。

- `bus`/`bicycle` 召回率为 0% 是 NuScenes→KITTI **domain gap** 问题（两个引擎均如此），与分辨率无关，需换训练数据集才能解决。
- `pedestrian` 召回率偏低（约 40%）同理，属模型限制而非分辨率问题。
...
|---------|---------|---------|----------|
| **降低分辨率至 320×960** | 实测 **-39%** 延迟 (101→62ms) | 已实现（build_engine.py --height 320 --width 960） | ✅ 显著有效（骨干+FPN全面加速）|
...
**结论**：降分辨率至 320×960 单项即可达到 62ms（超过 50ms 目标的 80% 进展）。若 320×960 精度满足要求，则已接近目标；若叠加 cuBLASLt 仍不够，下一步为 **更换模型架构**（YOLOv8 系列可达 8~15ms）。
