...
> 注：bench_single.py 测量纯 GPU 推理时间（不含预处理），代表 C++ 节点的推理延迟参考值。
...
| FP16_375h+cuBLASLt | 375×1242 | FP16 | **90ms** | 1.12x（快11%）|
| FP16_320×960（非等比） | 320×960 | FP16 | **62ms** | **1.63x（快39%）** |
| FP16_320×1060（等比） | 320×1060 | FP16 | **65ms** | 1.55x（快36%）|

#### ⑥ 检测精度对比（KITTI 100张，IoU≥0.3，thr=0.30）

| 引擎 | 分辨率策略 | Recall | Precision | F1 | GPU推理P50 | car | truck | ped |
|------|--------|--------|-----------|-----|-----------|-----|-------|-----|
| FP16_375h（基准） | 训练尺寸 | 67.4% | 47.0% | 55.4% | 101ms | 78.1% | 66.0% | 43.1% |
| FP16_320×960 | 非等比（宽高比-9.4%） | 70.3% | 49.9% | 58.4% | 62ms | 82.1% | 69.8% | 41.2% |
| **FP16_320×1060** | **等比缩放** | **67.6%** | **50.1%** | **57.5%** | **65ms** | **78.8%** | **64.2%** | **43.1%** |

**结论：**
- **320×1060（等比）** 在 car/pedestrian 上与基准基本持平（宽高比误差仅 0.015%），是符合規范的部署方案
- **320×960（非等比）** F1 反而更高是偶然：宽度被额外压缩导致部分误检减少（Precision 表面升高），不代表真实精度提升
- `bus`/`bicycle` 0% 是 NuScenes→KITTI **domain gap**，与分辨率无关
...
|---------|---------|---------|----------|
| **降低分辨率 320×1060（等比）** | 实测 **-36%** 延迟 (101→65ms) | 已实现（--height 320 --width 1060） | ✅ **推荐方案**（宽高比误差仅 0.015%）|
| **降低分辨率 320×960（非等比）** | 实测 **-39%** 延迟 (101→62ms)，F1 +3% 偶然 | 已实现（--height 320 --width 960） | ⚠️ 不规范（宽高比失真 9.4%）|
| **cuBLASLt 战术** | 实测 **-11%** 延迟 (101→90ms) | 已实现（--cublas-lt） | ✅ 有效（FC层GEMM优化）|
...
**结论**：**320×1060 等比引擎**是当前最优选择：65ms、宽高比与训练基本一致、精度与基准持平。若进一步要低于 50ms，需更换模型架构（YOLOv8 系列）。
